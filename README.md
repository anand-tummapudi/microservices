...
* POST creates a new resource, which can be then deleted by using DELETE. GET retrieves the current state of a resource in some representation. PUT transfers a new state onto a resource.
...
#### 34. Grafana
- DevOps Life cycle - Continuous Development -> Continuous Integration -> Continuous Testing -> Continuous Deployment -> Continuous Monitoring
- Continuous monitoring is the crucial phase of the devops where we monitor the performance of your application. Vital information of the system is monitored, also the root cause of the issue can be identified. Security and availability of the services. Also if there are any network issues that also can be resolved. It basically helps us in fixing the issue as soon as it is detected.
- Continous Monitoring comes up with various stages. i. Monitoring tool (San sui, Prometheus, nagios) ii. Configuration management tool (ansible, chef, puppet) iii. Alering Tool(Pager Duty, Service Now , Slack ) iv. Metric Storage(influxdb, splunk, aws) v. Visualization tool(Grafana).
- **Continuous monitoring** tools are sensu, nagios, prometheus. Sen sui is sensible and scalable telemetry and service health checking solution for monitoring servers, container services, applications functions and connected services. Prometheus relies on the pull method to collect information with the built in database. Nagios is the legacy monitoring tool that introduced monitoring practices to a generation of operators.
...
- Grafana is expandable through a plugin system it allows you to query, visualize alert on and understand your metrics no mattter. We can create, explore and share dashboard with team and foster a data driven culture.
- Grafana has become most popular tool which is used to compose observability dashboards with everything from prometheus and graphite metrics to logs and application data to power plats and beehives.
- Features of Grafana i. Visualization - From heat maps to histograms and graphs to geo maps it has a plethora of visualization options to help you understand your data. ii. Alerts - It seamlessly define alerts where it makes sense while you are in the data.define threasholds visually and get notified via slack and pagerduty. Grafana supports many databases and the beaty is that you can bring all of them to the same dashboard.
